...
pub fn get_package(pkg_app_id: String, pkg_branch: String, pkg_remote: String) -> Result<Option<Package>, diesel::result::Error> {
    use crate::database::schema::appstream_packages::dsl::*;
    let con = connect_db!();

    let mut packages = appstream_packages.filter(app_id.eq(pkg_app_id)).filter(branch.eq(pkg_branch)).filter(remote.eq(pkg_remote)).load::<DbPackage>(&con)?;
